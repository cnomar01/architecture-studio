...
export default function CalendarPage(){
 const now=new Date(); const [cursor,setCursor]=useState(new Date(now.getFullYear(),now.getMonth(),1)); const [events,setEvents]=useState<CalendarEvent[]>([]); const [error,setError]=useState("");
 useEffect(()=>{getCalendarEvents().then(setEvents).catch((reason)=>setError(reason instanceof Error?reason.message:"Could not load calendar events."));},[]);
 ...
 return <main className="min-h-screen bg-[#111] px-6 py-10 text-white lg:px-10"><div className="mx-auto max-w-7xl">
  ...
  <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_320px]">
   ...
   <aside className="rounded-3xl border border-white/10 bg-white/[.03] p-5"><div className="flex items-center gap-2"><Clock size={16} className="text-white/35"/><h2 className="font-medium">Upcoming</h2></div><div className="mt-5 space-y-3">{[...events].sort((a,b)=>a.date.localeCompare(b.date)).slice(0,8).map(e=><div key={e.id} className="rounded-2xl border border-white/10 p-4"><div className="text-sm font-medium">{e.title}</div><div className="mt-2 text-xs text-white/35">{e.date}{e.time?` · ${e.time}`:""}</div><div className="mt-2 flex items-center gap-2 text-[10px] text-white/25"><span>{e.type}</span><span>·</span><span>{e.project}</span></div></div>)}</div>{error&&<p className="py-4 text-sm text-red-300">{error}</p>}{!error&&events.length===0&&<p className="py-8 text-center text-sm text-white/25">No scheduled events.</p>}</aside>
  </div>
 </div></main>
}
