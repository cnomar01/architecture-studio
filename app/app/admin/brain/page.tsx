      ...
      <div className="mx-auto max-w-6xl px-5 py-8 md:px-8">
        <header className="mb-6 flex flex-col gap-5 border-b border-black/10 pb-7 md:flex-row md:items-end md:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex items-center gap-2 rounded-full border border-black/10 px-4 py-2 text-xs font-medium"><span className={`h-2 w-2 rounded-full ${aiStatus?.ollama ? "bg-emerald-500" : "bg-black/25"}`} /> Local AI · {aiStatus?.ollama ? aiStatus.model : "Offline"}</div>
        ...
        {mode === "engineer" ? (
          <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
            <main className="rounded-2xl border border-black/10 bg-[#fafafa] p-4 md:p-6">
              <div className="mb-5 flex items-center gap-3"><div className="grid h-11 w-11 place-items-center rounded-xl bg-black text-white"><Brain size={20} /></div><div className="min-w-0 flex-1"><div className="font-semibold">Owner Mode</div><div className="text-xs text-black/50">Private local intelligence · no OpenAI API credits required.</div></div><div className={`hidden items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] sm:flex ${aiStatus?.ollama ? "border-black/10 text-black/55" : "border-black/10 text-black/35"}`}><span className={`h-1.5 w-1.5 rounded-full ${aiStatus?.ollama ? "bg-emerald-500" : "bg-black/20"}`} /> {aiStatus?.ollama ? "Connected" : "Start Ollama"}</div></div>
