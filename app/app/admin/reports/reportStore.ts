...
export async function getManagementReport(
  ...
  const overloadedTeam = team.filter(
  ...
  const projectRows = projects.map((project) => {
    ...
    const projectIssues = siteReports
      .filter((report) => matchesProject(report, project.id))
      .flatMap((report) => report.issues || []);

    const projectFinance = transactions.filter(
    ...
    const projectIncome = projectFinance
      .filter((item) => item.type === "Income")
      .reduce((sum, item) => sum + item.amount, 0);

    const projectExpenses = projectFinance
      .filter((item) => item.type === "Expense")
      .reduce((sum, item) => sum + item.amount, 0);
    ...
    return {
      ...
      progress: projectTasks.length
        ...
        : 0,
      openTasks: projectTasks.filter(
        (task) => task.status !== "Completed"
      ).length,
      pendingApprovals: projectApprovals.filter(
        (item) => item.status === "Pending"
      ).length,
      openIssues: projectIssues.filter(
