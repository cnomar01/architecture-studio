...
const readFile = (file: File) => new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onerror = () => reject(new Error("Could not read this file.")); reader.onload = () => resolve(String(reader.result)); reader.readAsDataURL(file); });
...
export default function AdminFilesPage() {
  ...
  function beginNew() { const next = emptyFile(); if (projects[0]) { next.project_id = projects[0].id; next.project_name = projects[0].name; } setMessage(""); setError(""); setEditing(next); }
  function beginEdit(file: ProjectFile) { setMessage(""); setError(""); setEditing({ ...file, tags: [...(file.tags || [])] }); }
  function beginRevision(file: ProjectFile) { const next = { ...file, id: "", revision: nextRevision(file.revision), parent_file_id: file.parent_file_id || file.id, is_current: true, issue_date: new Date().toISOString().slice(0, 10) }; setMessage(""); setError(""); setEditing(next); }
  ...
  async function attach(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (!selected) return;
    if (selected.size > 4 * 1024 * 1024) { setError("Files saved in the workspace are limited to 4 MB each."); return; }
    try { const data = await readFile(selected); setEditing((current) => current ? { ...current, storage_key: data, file_name: selected.name, file_type: selected.type || "application/octet-stream", file_size: selected.size } : current); } catch (cause) { setError(cause instanceof Error ? cause.message : "Could not attach this file."); }
  }

  async function save(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (!editing) return;
    setError(""); setMessage("");
    if (!editing.name.trim() || !editing.project_id) { setError("Choose a project and enter a file name."); return; }
    setSaving(true);
    try {
      const project = projects.find((item) => item.id === editing.project_id);
      const payload = { ...editing, id: editing.id || `FIL-${crypto.randomUUID().slice(0, 8).toUpperCase()}`, project_name: project?.name || editing.project_name || "", tags: editing.tags.map((tag) => tag.trim()).filter(Boolean) };
      if (!editing.id && editing.parent_file_id) await fetch("/api/data/files", { method: "PATCH", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify({ id: editing.parent_file_id, is_current: false }) });
      const response = await fetch("/api/data/files", { method: editing.id ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify(payload) });
      const data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data.error || "Could not save this file.");
      setEditing(null); setMessage(editing.id ? "File updated." : editing.parent_file_id ? "New revision created." : "File uploaded to the shared workspace."); await load();
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Could not save this file."); }
    finally { setSaving(false); }
  }

  async function remove(file: ProjectFile) {
    if (!window.confirm(`Delete “${file.name}” (${file.revision})? This cannot be undone.`)) return;
    setError(""); const response = await fetch(`/api/data/files?id=${encodeURIComponent(file.id)}`, { method: "DELETE", credentials: "include" }); const data = await response.json().catch(() => ({}));
    if (!response.ok) { setError(data.error || "Could not delete this file."); return; }
    setFiles((current) => current.filter((item) => item.id !== file.id)); setMessage("File deleted.");
  }
