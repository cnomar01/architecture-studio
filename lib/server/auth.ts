...
export type ServerUser = {
  id: string; name: string; email: string; role: "Owner" | "Manager" | "Engineer" | "Client"; employeeId?: string; clientId?: string; active: boolean;
};

function mapUser(row: any): ServerUser {
  return { id: row.id, name: row.name, email: row.email, role: row.role, employeeId: row.employee_id || undefined, clientId: row.client_id || undefined, active: Boolean(row.active) };
}
...
export async function getServerUser(): Promise<ServerUser | null> {
  ...
  await query("UPDATE sessions SET last_seen_at=NOW() WHERE id=$1", [sessionId]).catch(() => {});
  return mapUser(result.rows[0]);
}

export async function requireServerUser(roles?: ServerUser["role"][]) {
...
export async function audit(action: string, entityType: string, entityId?: string, metadata: Record<string, unknown> = {}) {
  ...
  await query("INSERT INTO audit_logs(actor_user_id,actor_name,action,entity_type,entity_id,metadata,ip_address,user_agent) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)", [user?.id || null, user?.name || "System", action, entityType, entityId || null, JSON.stringify(metadata), ip, ua]);
}
