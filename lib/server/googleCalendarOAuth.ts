...
export const GOOGLE_CALENDAR_SCOPES =
  `openid email ${GOOGLE_CALENDAR_EVENTS_SCOPE} ${GOOGLE_DRIVE_FILE_SCOPE}`;
...
export async function exchangeGoogleCalendarCode(
  ...
  if (!tokenResponse.ok || !token.access_token) {
    throw new GoogleCalendarOAuthError(
      "Google authorization could not be completed. Start again from Connect Google."
    );
  }

  const grantedScopes = token.scope?.split(/\s+/) ?? [];

  if (!grantedScopes.includes(GOOGLE_CALENDAR_EVENTS_SCOPE)) {
    throw new GoogleCalendarOAuthError(
      "Calendar event permission was not granted. Reconnect and allow calendar access."
    );
  }
  ...
  if (!token.refresh_token) {
    throw new GoogleCalendarOAuthError(
      "Google did not grant offline access. Start again from Connect Google."
    );
  }
  ...
  if (
    ...
    throw new GoogleCalendarOAuthError(
      "Google could not verify the connected account email."
    );
  }
